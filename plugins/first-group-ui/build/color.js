...
function getColor(line) {
  let arr = line.split(':')
  let colorName = arr[0].trim()
  return [fmtColorName(colorName), arr[1].trim().replace(/;/g, ''), '$--color-' + colorName]
}
...
function createColorJSFile(theme) {
  // resource scss file
  const COLOR_SASS_FILE = resolve(__dirname, `../src/assets/scss/theme-${theme}.scss`)
  ...
  try {
    ...
    let tempArr
    const colorsArray = sassData.toString().split(new RegExp(os.EOL)).reduce((prev, line) => {
      line = line.trim()
      if (/\$-+color-(.+)/.test(line)) {
